...
struct GpuOnPairChunk {
    ...
    total_tokens: usize,
    dict_max_len: u8,
    /// Largest decoded batch per warp-chunk size, straight off the offset sidecar: consecutive
    /// offsets bracket one batch, so their difference is exactly what that batch stages. This
    /// is what makes a staging buffer smaller than the 16-bytes-per-token worst case provably
    /// safe for a given column, with no runtime search and no overflow path in the kernel.
    max_batch_bytes: Vec<(usize, u64)>,
...
struct KernelVariant {
    ...
    chunk_size: usize,
    block_warps: u32,
    /// Staging bytes reserved per token (the kernel's ONPAIR_STAGE_BYTES). `None` means the
...
#[cfg(feature = "cuda")]
const PRESET_NAMES: &[&str] = &["tpt-matched", "packed-grid", "packed-s", "production"];
...
fn select_gpu_kernels(requested: Option<&[String]>) -> Result<Vec<KernelVariant>> {
    ...
    let expanded: Vec<&str> = if requested == ["tpt-matched"] {
        ...
        // Every deployable kernel and nothing else. This exists for clock sweeps: a wide spread of
        ...
        anyhow::ensure!(
            !requested
                .iter()
                .any(|name| PRESET_NAMES.contains(&name.as_str())),
            "a kernel preset cannot be combined with individual kernel names"
        );
...
/// spends any headroom on more blocks, so shrinking the staging buffer raises occupancy and
/// leaves cache capacity unchanged -- which is the opposite of what the dictionary gather
/// wants. This is the only knob that makes the split an experimental variable rather than a
/// by-product.
///
...
fn apply_carveout_policy(function: &cudarc::driver::CudaFunction) -> Result<Option<i32>> {
    ...
    let pct: i32 = match raw.trim().to_ascii_lowercase().as_str() {
        "" | "default" => -1,
        "maxl1" => 0,
        "maxshared" => 100,
    ...
    anyhow::ensure!(
        (-1..=100).contains(&pct),
        "ONPAIR_CARVEOUT percentage out of range: {pct}"
    );
    ...
    function
        ...
        .map_err(|e| anyhow::anyhow!("set preferred shared-memory carveout: {e:?}"))?;
    Ok(Some(pct))
}
...
fn inapplicable_reason(
    ...
    // A staging-bytes variant is safe exactly when the column's LARGEST batch fits its buffer.
    // The offset sidecar already answers that: chunk_offsets[c+1] - chunk_offsets[c] is batch
    // c's decoded size, so the bound is known without decoding anything. Kernels whose buffer
    // is too small for this column are reported inapplicable rather than run, which is why the
    // decoder needs no overflow path.
    if let Some(stage_bytes) = variant.stage_bytes {
        let cap = variant.chunk_size * stage_bytes as usize + 32;
        // +15 covers the shift that aligns scratch index to output address modulo 16.
        let worst = chunks
            .iter()
            .filter_map(|c| {
                c.max_batch_bytes
                    .iter()
                    .find(|(n, _)| *n == variant.chunk_size)
                    .map(|(_, v)| *v)
            })
            .max();
        if let Some(worst) = worst {
            if worst as usize + 15 > cap {
                return Some(format!(
                    "staging {cap} B/warp is below this column's largest batch ({worst} B + 15 shift)"
                ));
            }
        }
    }
