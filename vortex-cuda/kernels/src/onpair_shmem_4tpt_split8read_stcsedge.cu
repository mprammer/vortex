...
// Uniform streaming drain policy (2026-08-11): the shipped kernel applies __stcs to
// the aligned 16-byte body store only; the head and tail go out as plain byte stores.
// This variant gives them an evict-first policy too, so the whole drain has one cache
// policy. Head plus tail is at most 15 + 15 bytes per warp against ~768 B of output
// at mean token length 6, so the bytes are ~4% — the question is whether keeping
// write-once output lines out of L1 is worth anything to the gather that shares it.
//
// Byte-exact with the shipped kernel by construction: same bytes, same addresses,
// only the cache policy differs.
//
...
__device__ inline uint32_t warp_inclusive_scan_u32_s8rce(uint32_t x, int lane) {
...
extern "C" __global__ ONPAIR_LAUNCH_BOUNDS void onpair_shmem_4tpt_split8read_stcsedge(
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        const uint32_t incl = warp_inclusive_scan_u32_s8rce(l[k], lane);
...
#pragma unroll
    ...
    if ((uint32_t)lane < head) {
        st_cs_u8(&output_bytes[out_start + (uint64_t)lane], s_buf[lane]);
    }
    if (head >= warp_total) {
        return;
    }
    ...
    if ((uint32_t)lane < warp_total - tail_start) {
        st_cs_u8(&output_bytes[out_start + (uint64_t)tail_start + (uint64_t)lane],
