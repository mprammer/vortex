...
//
// Why this variant and not the existing `onpair_shmem_4tpt_ldcs`: that one applies
// the hint to the stride-16 base, where it is worth ~1% in geomean over 167
// committed cells and -5.6% on the L40S. `split8read` already halves the hot dict
// footprint, which is the same L1 pressure the hint relieves, so the two may not
// compose. This measures whether any headroom is left. Only the codes load
// differs from the shipped kernel; scan, gather, emit and drain are identical.
//
...
__device__ inline uint32_t warp_inclusive_scan_u32_s8rldcs(uint32_t x, int lane) {
...
extern "C" __global__ ONPAIR_LAUNCH_BOUNDS void onpair_shmem_4tpt_split8read_ldcs(
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        ...
        if (i < total_tokens) {
            const uint32_t code = (uint32_t)__ldcs(&codes[i]);
            c[k] = code;
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        const uint32_t incl = warp_inclusive_scan_u32_s8rldcs(l[k], lane);
